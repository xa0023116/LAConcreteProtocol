...
typedef void (^la_specialProtocolInjectionBlock)(Class);

// a `const char *` equivalent to system struct objc_method_description
typedef struct {
    SEL name;
    const char *types;
} la_methodDescription;
...
static LASpecialProtocol * restrict specialProtocols = NULL;
 ...
 */
static void la_injectSpecialProtocols (void) {
    /*
     ...
     */
    qsort_b(specialProtocols, specialProtocolCount, sizeof(LASpecialProtocol), ^(const void *a, const void *b){
        // if the pointers are equal, it must be the same protocol
        if (a == b)
            return 0;

        const LASpecialProtocol *protoA = a;
        const LASpecialProtocol *protoB = b;

        // A higher return value here means a higher priority
        int (^protocolInjectionPriority)(const LASpecialProtocol *) = ^(const LASpecialProtocol *specialProtocol){
            int runningTotal = 0;
    ...
    @autoreleasepool {
        ...
        for (size_t i = 0;i < specialProtocolCount;++i) {
            ...
            // from the structure
            la_specialProtocolInjectionBlock injectionBlock = (__bridge_transfer id)specialProtocols[i].injectionBlock;
            ...
            for (unsigned classIndex = 0;classIndex < classCount;++classIndex) {
                ...
                bool isConformsProtocol = class_conformsToProtocol(class, protocol);
                
                //  解决任何实现协议的子类被排除的情况， 但是向上寻找父类的效率极其可怕，无法这样去实现
                //                    // 向上寻找父类
                //                    Class clazz = class;
                //                    while (!isConformsProtocol && (clazz = class_getSuperclass(clazz))) {
                //                        isConformsProtocol = class_conformsToProtocol(clazz, protocol);
                //                    }
                
                if (!isConformsProtocol)
                    continue;
...
unsigned la_injectMethods (
    Class aClass,
    Method *methods,
    unsigned count,
    la_methodInjectionBehavior behavior,
    la_failedMethodCallback failedToAddCallback
) {
    ...
    @autoreleasepool {
        ...
        if (!isMeta) {
            ...
            behavior &= ~(la_methodInjectionIgnoreLoad | la_methodInjectionIgnoreInitialize);
        }
        ...
        for (unsigned methodIndex = 0;methodIndex < count;++methodIndex) {
            ...
            if (behavior & la_methodInjectionIgnoreLoad) {
            ...
            if (behavior & la_methodInjectionIgnoreInitialize) {
            ...
            switch (behavior & la_methodInjectionOverwriteBehaviorMask) {
            case la_methodInjectionFailOnExisting:
                success = class_addMethod(aClass, methodName, impl, type);
                break;

            case la_methodInjectionFailOnAnyExisting:
            ...
            case la_methodInjectionReplace:
                class_replaceMethod(aClass, methodName, impl, type);
                break;

            case la_methodInjectionFailOnSuperclassExisting:
                {
...
BOOL la_injectMethodsFromClass (
    Class srcClass,
    Class dstClass,
    la_methodInjectionBehavior behavior,
    la_failedMethodCallback failedToAddCallback)
{
    ...
    // ignore +load
    behavior |= la_methodInjectionIgnoreLoad;
    addedCount = la_injectMethods(
...
Class la_classBeforeSuperclass (Class receiver, Class superclass) {
...
Class *la_copyClassList (unsigned *count) {
...
unsigned la_addMethods (Class aClass, Method *methods, unsigned count, BOOL checkSuperclasses, la_failedMethodCallback failedToAddCallback) {
    la_methodInjectionBehavior behavior = la_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= la_methodInjectionFailOnSuperclassExisting;

    return la_injectMethods(
        aClass,
        methods,
        count,
        behavior,
...
BOOL la_addMethodsFromClass (Class srcClass, Class dstClass, BOOL checkSuperclasses, la_failedMethodCallback failedToAddCallback) {
    la_methodInjectionBehavior behavior = la_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= la_methodInjectionFailOnSuperclassExisting;
    
    return la_injectMethodsFromClass(srcClass, dstClass, behavior, failedToAddCallback);
}

BOOL la_classIsKindOfClass (Class receiver, Class aClass) {
    while (receiver) {
        if (receiver == aClass)
            return YES;
...
Class *la_copyClassListConformingToProtocol (Protocol *protocol, unsigned *count) {
    Class *allClasses;
    ...
    @autoreleasepool {
        ...
        allClasses = la_copyClassList(&classCount);
...
la_propertyAttributes *la_copyPropertyAttributes (objc_property_t property) {
    ...
    la_propertyAttributes *attributes = calloc(1, sizeof(la_propertyAttributes) + typeLength + 1);
    if (!attributes) {
    ...
    while (next && *next == ',') {
        ...
        case 'C':
            attributes->memoryManagementPolicy = la_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = la_propertyMemoryManagementPolicyRetain;
            break;
...
Class *la_copySubclassList (Class targetClass, unsigned *subclassCount) {
    unsigned classCount = 0;
    Class *allClasses = la_copyClassList(&classCount);
...
Method la_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
...
BOOL la_getPropertyAccessorsForClass (objc_property_t property, Class aClass, Method *getter, Method *setter) {
    la_propertyAttributes *attributes = la_copyPropertyAttributes(property);
    if (!attributes)
        return NO;
...
NSMethodSignature *la_globalMethodSignatureForSelector (SEL aSelector) {
    ...
    static la_methodDescription volatile methodDescriptionCache[selectorCacheLength];

    uintptr_t hash = (uintptr_t)((void *)aSelector) & selectorCacheMask;
    la_methodDescription methodDesc;
    ...
    methodDesc = (la_methodDescription){.name = NULL, .types = NULL};

    uint classCount = 0;
    Class *classes = la_copyClassList(&classCount);
    ...
    if (classes) {
        @autoreleasepool {
            ...
            for (uint i = 0;i < classCount;++i) {
                ...
                if (method) {
                    methodDesc = (la_methodDescription){.name = aSelector, .types = method_getTypeEncoding(method)};
                    break;
                }
            }
        }
    ...
    if (!methodDesc.name) {
        ...
        if (protocols) {
            ...
            for (uint i = 0;i < protocolCount;++i) {
                ...
                if (objcMethodDesc.name) {
                    methodDesc = (la_methodDescription){.name = objcMethodDesc.name, .types = objcMethodDesc.types};
                    break;
                }
            }
            free(protocols);
...
BOOL la_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        la_specialProtocolInjectionBlock copiedBlock = [injectionBehavior copy];

        // construct a new EXTSpecialProtocol structure and add it to the first
        // empty space in the array
        specialProtocols[specialProtocolCount] = (LASpecialProtocol){
            .protocol = protocol,
...
void la_specialProtocolReadyForInjection (Protocol *protocol) {
    @autoreleasepool {
        NSCParameterAssert(protocol != nil);
...
void la_removeMethod (Class aClass, SEL methodName) {
    Method existingMethod = la_getImmediateInstanceMethod(aClass, methodName);
    if (!existingMethod) {
        return;
    }
...
void la_replaceMethods (Class aClass, Method *methods, unsigned count) {
    la_injectMethods(
        aClass,
        methods,
        count,
        la_methodInjectionReplace,
        NULL
    );
}

void la_replaceMethodsFromClass (Class srcClass, Class dstClass) {
    la_injectMethodsFromClass(srcClass, dstClass, la_methodInjectionReplace, NULL);
}

NSString *la_stringFromTypedBytes (const void *bytes, const char *encoding) {
    switch (*encoding) {
